...
const server = http.createServer(async (req, res) => {
  ...
  if (['/decode', '/pixel'].includes(url.pathname) && req.method === 'POST') {
    try {
      ...
      // Stream both stages so the local harness exercises production replacement behavior.
      res.setHeader('Content-Type', 'application/x-ndjson');
      const send = decoded => res.write(JSON.stringify({ type: 'image', preview: decoded.preview, source: decoded.image, frameId: decoded.frameId, name: entry.name, kind: config.format, display: config.format === 'CFA' && Core.isRgbir(config) ? config.display : 'rgb', sequence: seq }) + '\n');
      send(await previewDecoder.decode({ path: path.join(root, entry.path), config, readOffset: seq.readOffset, length: info.requiredBytes, locale }, send));
      res.end();
    } catch (error) { res.statusCode = 400; res.end(JSON.stringify({ type: 'invalid', message: error.message, error: error.message })); } return;
  }
...
async function render(reset=true,requestId){if(reset){active.epoch++;send('viewer',{type:'resetPlayback'});state()}document.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.id===active.id));if(!ready.viewer)return;const n=++rev;if(active.kind!=='BINARY'){send('viewer',{type:'image',source:'/fixtures/'+active.name,name:active.name,kind:active.kind,revision:n});return;}send('viewer',{type:'loading',preserveImage:!reset});const response=await fetch('/decode?lang=${locale}',{method:'POST',body:JSON.stringify({id:active.id,config:active.config,frame:active.frame,fps:active.fps,epoch:active.epoch})});const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='';function present(line){if(!line)return;const data=JSON.parse(line);if(n!==rev)return;if(!data.preview){pixelRevision=n;pixelFrameId=data.frameId}send('viewer',{...data,sequence:data.sequence?{...data.sequence,fps:active.fps}:undefined,revision:n,requestId})}for(;;){const {value,done}=await reader.read();buffer+=decoder.decode(value,{stream:!done});let end;while((end=buffer.indexOf('\\n'))>=0){present(buffer.slice(0,end));buffer=buffer.slice(end+1)}if(done){present(buffer);break}}}
